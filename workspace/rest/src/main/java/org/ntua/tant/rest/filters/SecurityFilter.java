			...
			if(authHeader != null && authHeader.size() > 0){
				...
				if (user.getId()!=0){
					if (password.equals(user.getPassword())){
						if ((requestContext.getMethod().equalsIgnoreCase("delete")||requestContext.getMethod().equalsIgnoreCase("post")||requestContext.getMethod().equalsIgnoreCase("put"))&&!user.getRole().equalsIgnoreCase("admin")){
